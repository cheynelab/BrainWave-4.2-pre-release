...
SAVE_SURFACE = uimenu(SURF_MENU,'label','Save Surface as Shape File...','enable','off','separator','on','callback',@saveSurfaceAsShape);
...
SPM_MENU = uimenu(NORM_MENU,'label','Update SPM Normalisation ...','enable','off','callback',@update_normalization_callback);
GOTO_MNI = uimenu(NORM_MENU,'label','Goto MNI coordinate ...','enable','off','callback',@gotoMNI_callback);
VIEW_SURFACE = uimenu(NORM_MENU,'label','View Surface (3D Coregistration)','separator','on','enable','off','callback',@view_surfaceCallback);
...
CURSOR_TEXT = uicontrol('Style','Text','FontSize',11,'Units','Normalized','Position',...
    ...
    function saveSurfaceAsShape(~,~)
        ...
        Nvertices = size(surface_points,1);
        ...
        headpts = pts*vox2ctf;             % convert to MEG coords
        clear pts;
        ...
        % since Fiducials indexed 1 to 256, subtract from 256
        na = na_RAS;
        le = le_RAS;
        re = re_RAS;
        
        na(2) = slice_dim(2) - na(2); le(2) = slice_dim(2) - le(2); re(2) = slice_dim(2) - re(2);
        na(3) = slice_dim(3) - na(3); le(3) = slice_dim(3) - le(3); re(3) = slice_dim(3) - re(3);

        r = questdlg('Save all surface points or convex hull as shape?','Save Surface', 'All points','Convex Hull','Convex Hull');
        if strcmp(r,'Convex Hull')
            % generate convex hull subset of points
        ...
        [name, ~] = uiputfile('*.shape','Save shape file as...');
        if isequal(name,0)
            return;
        end
        [~,filePrefix,~] = fileparts(name);
        
        bw_write_ctf_shape(filePrefix, headpts, na, le, re, mmPerVoxel);         
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function fsl_meshCallback(~,~)
        ...
        shapeFile = bw_FSLmesh2shape(meshFile, matFile, filePrefix, slice_dim);
...
set(save_changes, 'enable','off'); 
    ...
    function display_surface_mesh( surface )
        ...
        uimenu(SURF_FILE_MENU,'label','Load Scanned Surface (.stl or .obj) ...','callback',@load_meshfile_callback);    
        uimenu(SURF_FILE_MENU,'label','Load Polhemus Surface (.pos) ...','callback',@load_polhemus_callback);    
        uimenu(SURF_FILE_MENU,'label','Import/Edit STL/OBJ Surface ...','separator','on','callback',@import_stl_callback);
        uimenu(SURF_FILE_MENU,'label','Save MRI mesh ...','separator','on','callback',@save_vtkCallback);
        ...
        function save_vtkCallback( ~,~ )
                ...
                saveName = strrep(saveName_auto,'.mri','.vtk');
                ...
                bw_save_surface_vtk(surface.vertices, surface.faces, fullname);
        end
