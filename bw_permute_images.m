...
[~, fileType] = bw_read_SPM_file(file);
if (fileType == 0)
...
if perm_options.corrected
    ...
    for k=1:num_perm
        if (mod(k,8) == 1)
            fprintf('Computing permutation: %d of %d',k-1, num_perm);
        end
...
% change for version 4.0 - bw_mip_plot_4D now only takes imageset as input
imageList(1,:) = filename;
imageset.no_subjects = 1;
imageset.params = bw_setDefaultParameters;
imageset.no_images = size(imageList,1);

imageset.dsName{1} = '';
imageset.covDsName{1} = '';
imageset.isNormalized = 1;
imageset.imageList{1} = imageList;
imageset.cond1Label = 'Permuted Average';

imagesetName = sprintf('%s_IMAGES.mat', fileprefix);

fprintf('Saving image set information in %s\n', imagesetName);
save(imagesetName, '-struct', 'imageset');

bw_mip_plot_4D(imagesetName, threshold);
...
bw_mip_plot_4D(imagesetName, 1-alpha);
