...
useSAMBaselineCheck = uicontrol('Style','checkbox','FontSize',12,'Units','Normalized','Position',...
    [0.35 0.58 0.2 0.04],'String','Use for SAM baseline','HorizontalAlignment','Center','enable','off',...
   ...
   function cov_dropdown_callback(src,~)
    ...
    function set_data_params_callback(~,~)
       % open using selected - e.g., to import mesh etc...
        ...
        covList = study.conditions{covCondition};
        dsList = study.conditions{condition1};          % can use for contrast as well...
        dsName = char(dsList(selectedDatasetIndex));    
        covDs = char(covList(selectedDatasetIndex));  
    ...
    function set_SAM_fields(instr, instr2)
        set(ACTIVEWINDOW_LABEL,'enable',instr)
        ...
        set(BASELINEWINDOW_LABEL,'enable',instr2)
