...
% ERB controls
uicontrol('style','text','fontsize',11,'units','normalized','position',...
    [0.03 0.51 0.15 0.02],'string','Latencies / Time Windows','BackgroundColor','white','foregroundcolor','blue','fontweight','b');
annotation('rectangle',[0.02 0.25 0.6 0.27],'EdgeColor','blue');
annotation('rectangle',[0.03 0.43 0.12 0.0],'EdgeColor','blue');

uicontrol('Style','Text','FontSize',12,'Units','Normalized','Position',...
    [0.04 0.45 0.06 0.025],'String','ERB','FontWeight','b','Background','White','horizontalAlignment','left');

RADIO_RANGE=uicontrol('style','radiobutton','units','normalized','position',[0.07 0.45 0.12 0.03],...
    'string','','backgroundcolor','white','value',0,'callback',@RADIO_RANGE_CALLBACK);
TIME_RANGE_LABEL = uicontrol('Style','Text','FontSize',10,'Units','Normalized','Position',...
    [0.17 0.485 0.15 0.025],'String','Latency Range (s):','Background','White','HorizontalAlignment','Left');
LAT_START_LABEL=uicontrol('Style','Text','FontSize',10,'Units','Normalized','HorizontalAlignment','left','Position',...
    [0.17 0.455 0.06 0.02],'String','Start:','BackgroundColor','White');
LAT_END_LABEL=uicontrol('Style','Text','FontSize',10,'Units','Normalized','HorizontalAlignment','left','Position',...
    [0.27 0.455 0.06 0.02],'String','End:','BackgroundColor','White');
LAT_STEPSIZE_LABEL=uicontrol('Style','Text','FontSize',10,'Units','Normalized','HorizontalAlignment','left','Position',...
    [0.37 0.455 0.06 0.02],'String','Step:','BackgroundColor','White');

RADIO_LIST=uicontrol('style','checkbox','units','normalized','position',[0.46 0.46 0.08 0.03],...
...
PLOT_BEAMFORMER_START_LAT_EDIT=uicontrol('Style','Edit','FontSize',10,'Units','Normalized','Position',...
    [0.2 0.445 0.05 0.04],'String',params.beamformer_parameters.beam.latencyStart,'BackgroundColor','White','Callback',...
...
PLOT_BEAMFORMER_END_LAT_EDIT=uicontrol('Style','Edit','FontSize',10,'Units','Normalized','Position',...
    [0.3 0.445 0.05 0.04],'String',params.beamformer_parameters.beam.latencyEnd,'BackgroundColor','White','Callback',...
...
PLOT_BEAMFORMER_STEP_LAT_EDIT=uicontrol('Style','Edit','FontSize',10,'Units','Normalized','Position',...
    [0.4 0.445 0.05 0.04],'String',params.beamformer_parameters.beam.step,'BackgroundColor',...
...
MULTI_DS_SAM_CHECK=uicontrol('Style','checkbox','FontSize',12,'Units','Normalized','Position',...
    [0.2 0.71 0.18 0.04],'String','Use for SAM baseline only','HorizontalAlignment','Center',...
    ...
    function MULTI_DS_SAM_CALLBACK(src,~)
       ...
       if params.beamformer_parameters.multiDsSAM
           warndlg('Select this option to use covariance dataset for SAM baseline only ...'); 
       end
       init_ds_params(full_ds_filename);
       update_fields;
    end
...
uicontrol('Style','Text','FontSize',12,'Units','Normalized','Position',...
    [0.04 0.33 0.06 0.025],'String','SAM','FontWeight','b','Background','White','horizontalAlignment','left');
RADIO_Z=uicontrol('style','radiobutton','units','normalized','position',[0.07 0.37 0.1 0.03],...
    'string','Pseudo-Z','backgroundcolor','white','value',0,'callback',@RADIO_Z_CALLBACK);
RADIO_T=uicontrol('style','radiobutton','units','normalized','position',[0.07 0.33 0.1 0.03],...
    'string','Pseudo-T','backgroundcolor','white','value',0,'callback',@RADIO_T_CALLBACK);
RADIO_F=uicontrol('style','radiobutton','units','normalized','position',[0.07 0.29 0.1 0.03],...
...
ACTIVE_WINDOW_LABEL = uicontrol('Style','Text','FontSize',10,'Units','Normalized','Position',...
    [0.17 0.4 0.15 0.025],'String','Active Window (s):','Background','White','HorizontalAlignment','Left');
...
BASELINE_WINDOW_LABEL = uicontrol('Style','Text','FontSize',10,'Units','Normalized','Position',...
    [0.17 0.31 0.18 0.025],'String','Baseline Window (s):','Background','White','HorizontalAlignment','Left');
    ...
    function PLOT_BEAMFORMER_BUTTON_CALLBACK(~,~)
        ...
        if useCovDs 
            ...
            if ~strcmp(subjectID,covSubjectID)
                errordlg('Selected Dataset and Covariance Dataset must have same subject ID');
                return;
            end
             
            % ** note the .covarianceType flag appears to be no longer used, 
            % ie.., a covariance dataset is always specified and params.beamformer_parameters.multiDsSAM
            % flag is passed to indicate to use covariance for SAM baseline only
            % 
            if params.beamformer_parameters.multiDsSAM
                params.beamformer_parameters.covarianceType = 2;    % use different weights for baseline only
            else
                params.beamformer_parameters.covarianceType = 1;    % use common weights
            end                
        else
            params.beamformer_parameters.covarianceType = 0;        % use independent weights
        end  
    ...
    function update_fields
        ...
        if strcmp(params.beamformer_parameters.beam.use,'ERB') || strcmp(params.beamformer_parameters.beam.use,'ERB_LIST')
            ...
            if strcmp(params.beamformer_parameters.beam.use,'ERB')
                ...
                set(TIME_RANGE_LABEL, 'enable','on');
...
%                 set(COMPUTE_MEAN_CHECK,'enable','on')  
            ...
            elseif strcmp(params.beamformer_parameters.beam.use,'ERB_LIST')
                set(TIME_RANGE_LABEL, 'enable','off');
...
%                 set(COMPUTE_MEAN_CHECK,'enable','off')  
            ...
            % * change in 4.2
            % have to keep this enabled to use covDs (common weights)
            % for SAM images.
            set(RADIO_LIST, 'enable','off');
            set(TIME_RANGE_LABEL, 'enable','off');
            set(COV_DS_CHECK,'enable','on')             
            if useCovDs
                set(COV_DS_LISTBOX,'enable','on');
...
function init_ds_params(dsName)
    ...
    % ** change 4.2
    if (params.beamformer_parameters.beam.use == 'T' | params.beamformer_parameters.beam.use == 'F') ...
            & params.beamformer_parameters.multiDsSAM & useCovDs
