...
%
% GUI for viewing and editing digitized surface mesh files
% Can read .stl files or WaveFront .obj format
% 
% written by D. Cheyne, Hospital for Sick Children
...
if ~exist('file','var')
    fprintf('Mesh file (.stl or .obj) required ... \n');
    return;
end
...
uimenu(FILE_MENU,'label','Close','Callback','closereq','Accelerator','W','separator','on');    
       ...
       % set fiducials
        ...
        uicontrol('style','pushbutton','units','normalized','Position',...
            [0.02 0.5 0.12 0.04],'String','Set Bounding Box','BackgroundColor','white',...
            'FontSize',10,'ForegroundColor','black','callback',@crop_callback);    
        
        uicontrol('style','pushbutton','units','normalized','Position',...
            [0.02 0.64 0.12 0.04],'String','Revert to Original','BackgroundColor','white',...
            'FontSize',10,'ForegroundColor','red','callback',@reset);   
        
        sl(1)  = uicontrol('style','text','units', 'normalized',...
            'position',[0.02 0.465 0.20 0.03],'String','X Range',...
            'FontSize',11,'HorizontalAlignment','left','BackGroundColor', 'white');  
        sl(2)  = uicontrol('style','slider','units', 'normalized',...
            'position',[0.02 0.45 0.20 0.02],'min',bb(1),'max',bb(2),...
            'Value',bb(1), 'sliderStep', [0.01 0.01],'BackGroundColor','white','callback',@x1_slider_Callback);      
        sl(3)  = uicontrol('style','slider','units', 'normalized',...
            'position',[0.02 0.43 0.20 0.02],'min',bb(1),'max',bb(2),...
            'Value',bb(2), 'sliderStep', [0.01 0.01],'BackGroundColor','white','callback',@x2_slider_Callback);      

        sl(4)  = uicontrol('style','text','units', 'normalized',...
            'position',[0.02 0.395 0.20 0.03],'String','Y Range',...
            'FontSize',11,'HorizontalAlignment','left','BackGroundColor', 'white');  
        sl(5)  = uicontrol('style','slider','units', 'normalized',...
            'position',[0.02 0.38 0.20 0.02],'min',bb(3),'max',bb(4),...
            'Value',bb(3), 'sliderStep', [0.01 0.01],'BackGroundColor','white','callback',@y1_slider_Callback);      
        sl(6)  = uicontrol('style','slider','units', 'normalized',...
            'position',[0.02 0.36 0.20 0.02],'min',bb(3),'max',bb(4),...
            'Value',bb(4), 'sliderStep', [0.01 0.01],'BackGroundColor','white','callback',@y2_slider_Callback);      

        sl(7)  = uicontrol('style','text','units', 'normalized',...
            'position',[0.02 0.325 0.20 0.03],'String','Z Range',...
            'FontSize',11,'HorizontalAlignment','left','BackGroundColor', 'white');  
        sl(8)  = uicontrol('style','slider','units', 'normalized',...
            'position',[0.02 0.31 0.20 0.02],'min',bb(5),'max',bb(6),...
            'Value',bb(5), 'sliderStep', [0.01 0.01],'BackGroundColor','white','callback',@z1_slider_Callback);      
        sl(9)  = uicontrol('style','slider','units', 'normalized',...
            'position',[0.02 0.29 0.20 0.02],'min',bb(5),'max',bb(6),...
            'Value',bb(6), 'sliderStep', [0.01 0.01],'BackGroundColor','white','callback',@z2_slider_Callback);    
        ...
        function realign_callback(~,~)
            ...
            % now update fids to be in same coordinates as mesh         
        ...
        function reset(~,~) 
            ...
            % have to reset fiducials to be relative to original mesh
            tfids = [[20 0 0]; [0 20 0]; [0 -20 0]];
            realign_callback;
        ...
        function cropMesh
            ...
            % revert to original face list to get "non-edited" mesh
            faces = save_faces;
            
            % now have to put vertices back in current fiducial coordinates
...
%                 [mesh.faces, mesh.vertices, mesh.colors] = relabelFaces(faces,vertices,colors);
        ...
        function save_meshCallback( ~,~ )
                ...
                fullname = fullfile(pathname,filename);
                bw_save_surface_vtk(vertices, faces, fullname);
